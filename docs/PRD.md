...
| CRDT sync engine + realtime gateway | **Implemented v1 (Phase 7, D16):** the document page wires the real SyncSession to the Rust gateway — two-client collaboration, offline/reconnect reconciliation, duplicate-safe resends, gateway restart recovery, graceful drain, snapshot resync; candidate-bound local/realtime evidence is recorded in the canonical report. Secret-backed remote rendered-browser CI was intentionally removed from the current gate set; no remote authenticated-browser result is claimed. A production-shaped AWS deployment was exercised historically and is not running now. | `tests/realtime/e2e.test.ts`, `tests/realtime/reliability.test.ts`, `tests/sync/*.test.ts`, `tests/browser/`, `docs/audits/CANONICAL_RELEASE_REPORT.md` |
...
| Status truthfulness | Save/durability status UI states local-saved vs mirror-saved truthfully; never claims server/cloud save before the durable-ack point | Phase 7 product audit (SA-PRODUCT7) + unit tests for status mapping |
