...
| CRDT sync engine + realtime gateway | **Implemented v1 (Phase 7, D16):** the document page wires the real SyncSession to the Rust gateway — two-client collaboration, offline/reconnect reconciliation, duplicate-safe resends, gateway restart recovery, graceful drain, snapshot resync; candidate-bound local/realtime evidence is recorded in the canonical report, while the current trusted rendered-browser gate remains pending. A production-shaped AWS deployment was exercised historically and is not running now. | `tests/realtime/e2e.test.ts`, `tests/realtime/reliability.test.ts`, `tests/sync/*.test.ts`, `tests/browser/`, `docs/audits/CANONICAL_RELEASE_REPORT.md` |
