...
| CRDT sync engine + realtime gateway | **Shipped v1 (Phase 7, D16):** the document page wires the real SyncSession to the Rust gateway — two-client collaboration, offline/reconnect reconciliation, duplicate-safe resends, gateway restart recovery, graceful drain, snapshot resync; verified through the local/release harness and rendered-browser E2E. A production-shaped AWS deployment was exercised historically and is not running now. | `tests/realtime/e2e.test.ts`, `tests/realtime/reliability.test.ts`, `tests/sync/*.test.ts`, `tests/browser/`, final hardening report |
